...
class MsgPackSpec extends FlatSpec {
  ...
  "MsgPack" should "handle invalid msg pack message" in {
    ...
    val stingToByteArray = (str: String) => str.split(" ").map(Integer.parseInt(_, 16).toByte)

    val testValues: Array[Int] = Array(10,13)

    val data = "92 0a 0d" // Message pack hex val for the array [10,13]
    val convertedValues: Array[Int] = msgpackMapper.readValue(stingToByteArray(data), classOf[Array[Int]])
    ...
    val result: Array[Int] = msgpackMapper.readValue(stingToByteArray(invalidData), classOf[Array[Int]])// java.lang.OutOfMemoryError: Java heap space
